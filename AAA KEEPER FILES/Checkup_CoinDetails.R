...
GetAllDetails <- function() {
  ...
  # API call
  coin.details <- fromJSON(url)
  # Extract data from API response
  if (coin.details$Response != "Success") {
    message("[ERROR] Failed to download.")
    message(coin.details$Response,"\n\t",coin.details$Message)
    coin.data <- NA
  } else {
    coin.data <- coin.details$Data
    message("Download complete.")
  }
  
  # Contained coins?
  coinsInList <- names(coin.data)
  
  # (second data set)
  fullyMatchedCats <- RunMergeScript(n_Coins=1000)
  
  # Merge
  coinsInList <- data.frame(coinsInList)
  names(coinsInList) <- c("symb")
  merg <- merge(coinsInList,fullyMatchedCats,by=c("symb"))
  
  # HOW MANY CAN CC FIND?
  numCoinsInCCList <- length(coin.data)
  numCoinsInMyListCCMatched <- nrow(merg)
  cat("\n\n")
  cat(" # in CC list: \t", numCoinsInCCList,"\n")
  cat(" # CC matched: \t", numCoinsInMyListCCMatched,"\n")
  cat(" ----------------------------------","\n")
  
  # Fully matched coins
  matched.Indices <- which((names(coin.data) %in% merg$symb))
  symb_Matched <- coinsInList[matched.Indices,]
  symb_Matched
  cat(" Matched ",length(symb_Matched),"of",length(coin.data),"coins.","\n")
  
  
  # Get matching coins from list, add to new list
  All_Details <- list()
  iCount <- 1
  for (i in 1:length(coin.data)) {
    if (names(coin.data)[i] %in% symb_Matched) {
      All_Details[[iCount]] <- coin.data[[i]]
      iCount <- iCount + 1
    }
  }
  cat(" Verified",length(All_Details),"of",length(symb_Matched),"matched coins.","\n\n")
  
  if (length(All_Details) > 0) {
    # Element titles
    elemTitles <- names(All_Details[[1]])
    # Convert the list to a data frame
    df <- data.frame(matrix(unlist(All_Details), nrow=iCount, byrow=T),stringsAsFactors=FALSE)
    names(df) <- elemTitles
  } else {
    df <- c("Fail")
  }
...
###################################
#
MergeCCWithDetails <- function(iDets=GetAllDetails()) {
  
  n_Coins <- 500
  # Download CMC data
  cmc <- DownloadCMCData(n_Coins)
  
  # Merge iDets (CC) with CMC data
  names(iDets) <- c("symb","name","algo","prooftype","premined") # Prep names for merging
  iMerged <- merge(cmc,iDets,by="symb")
  
  # Return
  iMerged
}
#
mergie <- MergeCCWithDetails(myDetails)
mergie
#
###--(  mmCats no longer available  )--###   ! ! !
#
mergie2 <- merge(mergie, mmCats, by="symb")
mergie2
#
###################################
